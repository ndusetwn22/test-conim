...
class FaApprovalSsView extends React.Component {
  constructor(props){
    super(props);
    this.state = {
  ...
  if (this.state.alasanReturn == '' || this.state.alasanReturn == null) {return await this.handleAlert('isSweetAlert', true, 'Warning', 'Alasan Return Tidak Boleh Kosong', 'warning')}
    ...
    let sql = `
          ...
          returning id as return_id)
            ...
            INSERT INTO df_ss_history_approval
            (ss_id, approved_rejected_by_id, approval_rejected_status, create_by, create_date)
            VALUES((select return_id from ins1), `+this.state.myProfile.id+`, 'Returned by FA', `+this.state.myProfile.id+`, timezone('utc-7', now()));
        `
        // console.log('sql : ', sql)
      ...
      :
        ...
        >
          ...
          <ModalBody className="modal-dialog-centered">
            Apakah anda yakin menyetujui SS ?
          </ModalBody>
          <ModalFooter>
...
export default FaApprovalSsView
